...
import torchvision
import numpy as np
from bindsnet.learning import snn
from bindsnet.learning import functional as sf
from bindsnet.analysis import visualization as vis
from bindsnet.utils import Intensity2Latency, DoGKernel, Filter, CacheDataset
...
def main_function():
        ...
        kernels = [DoGKernel(7, 1, 2),
                   DoGKernel(7, 2, 1), ]
        filter = Filter(kernels, padding=3, thresholds=50)
        s1 = S1Transform(filter)

        data_root = "data"
        MNIST_train = CacheDataset(torchvision.datasets.MNIST(root=data_root, train=True, download=True, transform=s1))
        MNIST_test = CacheDataset(torchvision.datasets.MNIST(root=data_root, train=False, download=True, transform=s1))
